...
| ~~4~~ | ~~Whether the on-panel touchscreen consumes the stream.~~ | **SETTLED 2026-09-06** | Two independent ways. Static: `/tuxedo` contains **0** occurrences of `SimpleDebugger` and **0** of the boundary `EH912ZZ` (it has 22 other `.interface` strings, none of them this one) — a property of the binary, not of a moment. Empirical, and stronger: v13 has been live with the stream gated and `/tuxedo` has run continuously at the same pid with **0 restarts**. If the touchscreen consumed the stream anonymously it would have broken at the flash. |
| ~~5~~ | ~~What `203.0.113.248` is.~~ | **IDENTIFIED 2026-09-06** | Reverse DNS resolved it to the site's Zabbix monitoring server. Short `:80` connections are availability polls, not stream consumption, which matches the observed shape. It is **not** an unauthenticated stream consumer, so the patch does not break it: the cave compares `r4` against the SimpleDebugger EhDir (`0x55b59c`) and returns stock's verdict unchanged for every other dir. Worth a glance at the Zabbix item's configured URL before flashing if it was ever pointed at something under `/SimpleDebugger.interface/`. |
