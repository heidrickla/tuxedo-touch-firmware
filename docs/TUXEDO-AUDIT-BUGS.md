...
- `panelStatusCode` is `-1` when `PanelIsTalking()==0`, otherwise `GetOnlineStatus()` — printed with `%d`, so `-1` is literally `-1` on the wire. `PanelIsTalking()` is `myPanel+0x65`, set by the ECP receiver and cleared when it stops hearing the VISTA; `GetOnlineStatus()` is `myPanel+0x67`, the VISTA's own online byte (1..4, 1 = online) copied out of its status response by `HandlePanelStatus` @0x00589454 beside AC, battery and telco. Two facts, not one. Decompiled 2026-09-12.
- Only the CURRENT partition is sent (`GetCurrentPartition() == part` guards the whole function), so field 2 is never a partition number.
...
**Code 22 — `SERV_PANEL_OFFLINE_MSG_BROADCAST`**: `0:22:<FLAG><CSS><text>:<panelStatusCode>` — same shape, no separate hex field, and followed by **two** `0:-1:<FLAG><CSS><text>` copies where a 21 gets three (handler @0x0000d9c4: `mvn r7,#0`, `bprintf` @0xda40 and @0xda60; then `setPartStatus(22,…)`, `setpanelOnline(0)`). Emitted in place of the 21 by the same `/tuxedo` function whenever `GetOnlineStatus() != 1` — the VISTA calling itself busy/downloading/offline, which is independent of the `-1`. CONFIRMED from both binaries; never yet observed on the wire (the panel has not been offline during a capture). tuxweb relays it from v16 (`push.rs` 22 arm); v15 dropped it silently.
